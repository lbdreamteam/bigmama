var OtherPlayersManager = (function () {
    ...
    var onConnect = function (id, oldPos, nowPos) {
        ...
        gameInstance.otherPlayersW.postMessage({ event: 'connect', params: { id: id, oldPos: oldPos, nowPos: nowPos } });
    }

    var onDisconnect = function (id) {
        gameInstance.clientsList[id].kill();
        gameInstance.otherPlayersW.postMessage({ event: 'disconnect', params: id });
    };

    var update = function (nowPos) {
        gameInstance.otherPlayersW.postMessage({ event: 'update', params: nowPos });
    };
