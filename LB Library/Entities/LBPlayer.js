LBPlayer = function (gameInstance, Tx, Ty, graph) {
    ...
    //Props
    this.calls = []; //--> DA RIVEDERE TUTTA LA RECONCILIATION <--
        ...
        if (this.cKeyboardInput.detectInput(this.cursors) != 'null' && (this.cKeyboardInput.increment.x != 0 || this.cKeyboardInput.increment.y != 0)) {
            this.cMovement.move(
                ...
                function (context, increment) {
                    ...
                    context.calls.push({input: context.cKeyboardInput.inputString});
                    eurecaServer.clientHandler({ event: 'sendInput', params: { increment: increment, clientId: myId, callId: context.calls.lenght - 1} });
...
LBPlayer.prototype.updatePosition = function (x, y, callId) {
    ...
    this.calls.splice(callId, 1);
    for (var iCall in this.calls) {
        var tmp = this.cKeyboardInput.switchFunction(this.calls[iCall].input);
        increment.x += tmp.x;
        increment.y += tmp.y;
    }
